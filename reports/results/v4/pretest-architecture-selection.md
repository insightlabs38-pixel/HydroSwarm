...
| `plan_value`, `plan_validity`, 5 consequence proxies (Strategist) | trained, validated, **not runtime-enabled** | Phase 14 gate 7 (≥2 finalist seeds) — this pass's delta item's Section 7 work ran the second seed (`v4-strategist-heads-v4corpus-corrected-seed2`) and found the two seeds **agree closely** with each other (every metric within ~2-3% relative; gate 7's procedural requirement is satisfied — see `reports/results/v4/strategist-second-seed-promotion-decision.md`). Decision: keep learned prescreening disabled anyway, because what the two agreeing seeds agree ON is that `learned_prescreen` does not beat `deterministic_heuristic` ordering at the same 3-call exact-simulator budget (lower regret and higher oracle-match rate for the deterministic policy in both seeds). Deterministic heuristic ordering + exact WNTR remain the deployed policy. Either promote-or-not outcome was explicitly acceptable per the governing spec — this is not a partial/incomplete result, and it is not a seed-disagreement failure. |
| `ood_category` | present (architecturally constructed, forward-passes cleanly), **untrained this run — excluded from every governance set** | Phase 13 finding: the governed train split has zero `ood_class` coverage by design (OOD-extension populations are development-holdout-only), so the head never received a real training gradient this run (near-chance macro F1 0.095, the expected result of evaluating an effectively-randomly-initialized head) — textbook Phase-14-gate-3 fail. Deterministic OOD severity (3-level) remains sole authority. Section 18.1 preserved a one-retrain path (vocabulary/schema frozen, advisory field already wired) without promoting anything now. |
