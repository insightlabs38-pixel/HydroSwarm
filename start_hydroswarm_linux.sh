#!/usr/bin/env bash
# Native Linux launcher for HydroSwarm. Always uses the project-local
...
if [ ! -x "$VENV_PYTHON" ]; then
  echo "error: $VENV_PYTHON not found. Run ./setup_hydroswarm_linux.sh first." >&2
  exit 1
fi
...
if ! "$VENV_PYTHON" -m hydroswarm.cli self-test --human; then
  echo "" >&2
  echo "error: readiness self-test failed. Run ./setup_hydroswarm_linux.sh to diagnose and fix." >&2
  exit 1
fi
