...
The current API default serves V5 and `hydroswarm self-test --strict` validates the V5 release bundle. There is one packaging caveat worth knowing: `docker-compose.release.yml` is pinned to the historical `v0.1.0-hackathon` image, which predates the V5 bundle, so it is **not** the V5 launch path. The native setup helper also retains an older redundant V4 bundle precheck before its final V5 strict self-test. See [Installation](docs/INSTALLATION.md) for exact behavior.
