...
export const demoIncident: IncidentView = {
  ...
  // not generated by running any real checkpoint, so it says so honestly
  ...
  provenance: {
    ...
    // from any real network/schema/checkpoint/calibration artifact --
    // easy to mistake for genuine frozen-V4 provenance if ever compared
    // against reports/results/v4/architecture-freeze.json. Empty string
    // represents this fixture's genuinely unavailable hash provenance,
    // the same convention errorIncidentView() already uses for "no real
    // hash to show" (api/incident.ts) -- TechnicalDock's ProvenanceTab
    // already renders `|| null` as an honest "not available", not a
    // fabricated value. Never a copy of the real frozen V4 hashes, which
    // this hand-authored fixture was never actually generated against.
    networkHash: '',
    featureSchemaHash: '',
    modelCheckpointHash: '',
    // Genuinely tied to this fixture: the real CALIBRATION_SCHEMA_VERSION
    // constant (hydroswarm.calibration.conformal), not a fabricated string.
    calibrationVersion: 'hydroswarm-calibration-v1',
    calibrationHash: '',
    // The verifier's real simulator identity
    // (hydroswarm.simulation.wrapper.WrappedSimulator.simulator_name) --
    // 'wntr-epanet' was never a value the codebase actually produces.
...
export const demoAuthorityCertificates: DecisionCertificate[] = [
  {
    ...
    calibrated: true,
    suppressionReasons: [],
    // UI-11.1 §4: demoIncident.provenance.*Hash are now '' (genuinely
    // unavailable, see that fixture's comment) -- null is this type's own
    // "not available" sentinel, so these follow it directly rather than
    // propagating an empty string into a field that already has a proper
    // null convention.
    provenance: {
      model: null,
      calibration: null,
