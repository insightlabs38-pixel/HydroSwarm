"""M10.3A Strategist refit -- Level A: frozen-backbone, Strategist-
specialist-side training under the frozen protocol
(`docs/evaluation/HYDROCORE_V5_M10_3_STRATEGIST_PREFLIGHT_REFIT.md`,
`m10_3_refit_protocol.py`).
...
Level-A checkpoints; `run_m10_3_level_a_gate.py` (not this script) decides
representation sufficiency.

Writes:
  reports/evaluation/hydrocore-v5/m10/m10-3-refit/m10-3-refit-gradient-coverage.json
  reports/evaluation/hydrocore-v5/m10/m10-3-refit/m10-3-refit-level-a.json
  reports/evaluation/hydrocore-v5/m10/m10-3-refit/checkpoints/level-a-seed{seed}/
...
def _train_level_a(model: HydroCore, train_examples: list[CorpusExample], *, seed: int) -> dict[str, Any]:
...
def main() -> None:
    ...
    for seed in m10.SEEDS:
        ...
        allowlist = set(proto.LEVEL_A_PARAMETER_ALLOWLIST)
        shared_encoder_params = [name for name in allowlist if name.startswith("candidate_plan_encoder")]
        task_head_prefix = {
        ...
        parameter_groups = {
            task: shared_encoder_params + [name for name in allowlist if name.startswith(prefix)]
    ...
    doc = {
        "kind": "M10_3_REFIT_LEVEL_A_TRAINING",
    ...
    (M10_3_REFIT_DIR / "m10-3-refit-level-a.json").write_text(json.dumps(doc, indent=2, default=str) + "\n")
    (M10_3_REFIT_DIR / "m10-3-refit-gradient-coverage.json").write_text(json.dumps(gradient_coverage_docs, indent=2, default=str) + "\n")
