"""M10.3A Strategist refit -- Level-A representation-sufficiency gate
evaluation, per `docs/evaluation/HYDROCORE_V5_M10_3_STRATEGIST_PREFLIGHT_REFIT.md`
(criteria fixed BEFORE any competence number here was computed).
...
and evaluates them against the SAME validation population
`run_m10_3_level_a_train.py` built (same seed base/count -- rebuilt here,
deterministically, matching the M10.2 Scout refit gate script's own
established convention of rebuilding rather than persisting the corpus).

Writes:
  reports/evaluation/hydrocore-v5/m10/m10-3-refit/m10-3-refit-level-a-gate.json
"""
...
def main() -> None:
    ...
    for model_seed in m10.SEEDS:
        print(f"=== evaluating Level-A gate, seed {model_seed} ===", flush=True)
        model = HydroCore.from_variant(m10.S_VARIANT, use_adapters=False, **SHARED_MODEL_CONFIG)
        checkpoint_dir = M10_3_REFIT_DIR / "checkpoints" / f"level-a-seed{model_seed}"
    ...
    (M10_3_REFIT_DIR / "m10-3-refit-level-a-gate.json").write_text(json.dumps(doc, indent=2, default=str) + "\n")
